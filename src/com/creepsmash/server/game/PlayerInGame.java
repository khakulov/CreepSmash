...
import com.creepsmash.server.game.states.RunningGameState;
...
public class PlayerInGame {
	...
	private static enum AnticheatType { CREDIT, INCOME }
	...
	public PlayerInGame(Client client) {
		...
		this.anticheatCurrentMoney = IConstants.START_CREDITS;
	...
	public synchronized void anticheat_TowerBuilt(String towerType, int towerId, long roundID) {
		...
		this.anticheatTowerIdPrices.put(towerId, TowerType.valueOf(this.anticheatTowerId.get(towerId)).getPrice());
		synchronized (this.items) {
	...
	public synchronized void anticheat_TowerUpgraded(int towerId, long roundID) {
		String nameOfNextTower = TowerType.valueOf(this.anticheatTowerId.get(towerId)).getNext().name();

		this.anticheatTowerId.remove(towerId);
		this.anticheatTowerId.put(towerId, nameOfNextTower);

		int oldPrice = this.anticheatTowerIdPrices.get(towerId);
		int newPrice = TowerType.valueOf(this.anticheatTowerId.get(towerId)).getPrice();
		this.anticheatTowerIdPrices.remove(towerId);
	...
	public void anticheat_sentThisCreep(String creepType, long roundID) {
		synchronized (this.items) {
			items.add(new AnticheatItem(roundID, AnticheatType.CREDIT, -1*CreepType.valueOf(creepType).getPrice()));
			items.add(new AnticheatItem(roundID, AnticheatType.INCOME, CreepType.valueOf(creepType).getIncome()));
		}
	}
	...
	public void anticheat_receivedThisCreep(String creepType, long roundID) {
		synchronized (this.items) {
			items.add(new AnticheatItem(roundID, AnticheatType.CREDIT, CreepType.valueOf(creepType).getBounty()));
		}
	}
	...
	public void anticheat_transferThisCreep(String creepType, long roundID) {
		synchronized (this.items) {
			items.add(new AnticheatItem(roundID, AnticheatType.CREDIT, -1*CreepType.valueOf(creepType).getBounty()));
		}
	}
	...
	public synchronized void anticheat_kickAndBan(RunningGameState state) {
		...
		state.getGame().sendAll(
				new MessageMessage("System", "<span style=\"color:red;\">"
						+ this.getClient().getPlayerModel().getName()
						+ " was kicked by <b>System</b></span>"));
		state.getGame().sendAll(
				new MessageMessage("System", this.getClient().getPlayerModel().getName()
						+ " has left..."));
		...
		Player player = entityManager.find(Player.class, this.getClient()
