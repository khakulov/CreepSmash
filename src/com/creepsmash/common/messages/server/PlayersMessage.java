...
public class PlayersMessage extends ServerMessage {
	...
	private static final String REG_EXP_PLAYERS = "PLAYERS((\\sPLAYER\\s\"([^\"]+)\"\\s([0-9]+))*)";
	private static final String REG_EXP_PLAYER = "(\\s)*\"([^\"]+)\"\\s([0-9]+)(\\s)*";
	...
	private Hashtable<String, Integer> playerNames;
	...
	public PlayersMessage() {
		super();
		this.playerNames = new Hashtable<String, Integer>();
	}
	
	/**
	 * @param playerNames the playerNames to set. None of the names may be null.
	 */
	public PlayersMessage(Hashtable<String, Integer> playerNames) {
		super();
	 ...
	 */
	public Hashtable<String, Integer> getPlayerNames() {
	 ...
	 * @param playerNames the playerNames to set. None of the names may be null.
	 */
	public void setPlayerNames(Hashtable<String, Integer> playerNames) {
	...
	public String toString() {
	    ...
	    while (e.hasMoreElements()) {
	      ...
	      message_player += " \"" + Message.prepareToSend(key) + "\"";
	      message_player += " " + this.playerNames.get(key);
		...
		if (matcher.matches()) {
			...
			for (String player : splitPlayerMessagePart) {
				...
				if (matcher_player.matches()) {
					...
					this.playerNames.put(matcher_player.group(2), Integer.parseInt(matcher_player.group(3)));
